...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Calculate text size with 15.0f font manually
    ImFont* font = g.Font;
    const float font_size = 15.0f;
    const ImVec2 label_size_15 = font->CalcTextSizeA(font_size, FLT_MAX, 0.0f, label);

    ImVec2 size = CalcItemSize(size_arg, label_size_15.x + style.FramePadding.x * 2.0f, label_size_15.y + style.FramePadding.y * 2.0f);
    ...
    // Center the text
    ImVec2 text_pos = bb.Min + ImVec2((size.x - label_size_15.x) * 0.5f, (size.y - label_size_15.y) * 0.5f);
    window->DrawList->AddText(nullptr, 15.0f, text_pos, GetColorU32(ImGuiCol_Text), label);
...
struct ButtonState {
    ImVec4 ButtonColor, BorderColor, ShadowColor, TextColor;
    float timer;
    ButtonState() : ButtonColor(0, 0, 0, 0), BorderColor(0, 0, 0, 0), TextColor(0, 0, 0, 0), timer(0.0f) {}
};

bool ImGui::CustomButton(const char* label, const char* text, float sizeX, float sizeY)
{
    ...
    ImVec4 target = ImGui::IsItemActive() ? ImColor(60, 120, 255) : ImGui::IsItemHovered() ? ImColor(50, 100, 210) : ImColor(60, 120, 255);
    ImVec4 ShadowColorTarget = ImGui::IsItemActive() ? ImVec4(0.235294118, 0.470588235, 1.0, 1.0f) : ImGui::IsItemHovered() ? ImVec4(0.235294118, 0.470588235, 1.0, 1.0f) : ImVec4(0, 0, 0, 0);
    ImVec4 targetText = ImGui::IsItemActive() || ImGui::IsItemHovered() ? ImColor(255, 255, 255) : ImColor(230, 230, 230);
    ...
    window->DrawList->AddRectFilled(MIN, MAX, ImGui::GetColorU32(it_anim->second.ButtonColor), 5, 0);
...
bool ImGui::CustomInvisibleButton(const char* label, const char* text, float sizeX, float sizeY)
{
    ImGuiStyle& style = ImGui::GetStyle();
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiIO& io = ImGui::GetIO();
    ...
    ImVec4 targetText = ImGui::IsItemActive() || ImGui::IsItemHovered() ? ImColor(60, 120, 255) : ImColor(60, 120, 255);
    it_anim->second.TextColor = ImVec4(ImLerp(it_anim->second.TextColor.x, targetText.x, time * 6), ImLerp(it_anim->second.TextColor.y, targetText.y, time * 6), ImLerp(it_anim->second.TextColor.z, targetText.z, time * 6), ImLerp(it_anim->second.TextColor.w, targetText.w, time * 6));
...
void ImGui::DrawHeaderGradient(ImDrawList* draw, ImVec2 windowPos, ImVec2 windowSize, float headerHeight, const char* label)
{
    draw->AddRectFilled(
        windowPos,
        ImVec2(windowPos.x + windowSize.x, windowPos.y + headerHeight),
        ImColor(0.08f, 0.08f, 0.09f, 1.0f)
    );
    ...
    for (int i = 0; i < segments; i++)
    {
        ...
        float actualT1 = 0.62f + (t1 * 0.38f);
        float actualT2 = 0.62f + (t2 * 0.38f);

        auto getColorAtPosition = [](float t) -> ImU32 {
            if (t < 0.72f) {
                float localT = (t - 0.62f) / 0.1f;
                return ImColor(0.08f, 0.08f, 0.081f + localT * (0.108f - 0.081f), 1.0f);
            }
            else if (t < 0.80f) {
                float localT = (t - 0.72f) / 0.08f;
                return ImColor(0.08f + localT * 0.04f, 0.08f + localT * 0.04f, 0.108f + localT * 0.054f, 1.0f);
            }
            else if (t < 0.85f) {
                float localT = (t - 0.80f) / 0.05f;
                return ImColor(0.12f + localT * 0.06f, 0.12f + localT * 0.06f, 0.162f + localT * 0.063f, 1.0f);
            }
            else if (t < 0.90f) {
                float localT = (t - 0.85f) / 0.05f;
                return ImColor(0.18f - localT * 0.06f, 0.18f - localT * 0.06f, 0.225f - localT * 0.063f, 1.0f);
            }
            else if (t < 0.96f) {
                float localT = (t - 0.90f) / 0.06f;
                return ImColor(0.12f - localT * 0.04f, 0.12f - localT * 0.04f, 0.162f - localT * 0.054f, 1.0f);
            }
            else {
                float localT = (t - 0.96f) / 0.04f;
                return ImColor(0.08f, 0.08f, 0.108f - localT * 0.027f, 1.0f);
            }
            };

        ImU32 color1 = getColorAtPosition(actualT1);
        ImU32 color2 = getColorAtPosition(actualT2);

        draw->AddRectFilledMultiColor(
            ImVec2(windowPos.x + width * actualT1, windowPos.y),
            ImVec2(windowPos.x + width * actualT2, windowPos.y + headerHeight),
            color1, color2, color2, color1
        );
    }

    ImGui::SetCursorScreenPos(ImVec2(windowPos.x + 10, windowPos.y + 8));
...
bool ImGui::CustomCheckbox(const char* label, bool* v)
{
    ...
    ImColor newBlue(0x90, 0x9c, 0xff);
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 1.0f : 0.0f, 0.16f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.circleColor = ImLerp(it_anim->second.circleColor, *v ? newBlue : hovered ? ImColor(143, 151, 255, 255) : ImColor(50, 50, 50), ImGui::GetIO().DeltaTime * 6.f);
    it_anim->second.shadowColor = ImLerp(it_anim->second.shadowColor, *v ? newBlue : hovered ? ImColor(143, 151, 255, 255) : ImColor(0, 0, 0, 0), ImGui::GetIO().DeltaTime * 6.f);
    it_anim->second.rectColor = ImLerp(it_anim->second.rectColor, *v ? ImColor(27, 27, 27) : hovered ? ImColor(20, 20, 20, 255) : ImColor(22, 22, 22), ImGui::GetIO().DeltaTime * 6.f);
    it_anim->second.textColor = ImLerp(it_anim->second.textColor, *v ? ImColor(255, 255, 255) : hovered ? ImColor(150, 150, 150, 255) : ImColor(100, 100, 100), ImGui::GetIO().DeltaTime * 6.f);
    ...
    if (label_size.x > 0.0f)
        window->DrawList->AddText(nullptr, 15.0f, total_bb.Min, ImColor(it_anim->second.textColor), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}


bool ImGui::CustomButtonSingle(const char* label, bool* clicked)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    const float circle_sz = 16.0f; // small button circle
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ...
    bool hovered, held;
    ImRect circle_bb(
        ImVec2(pos.x + w - circle_sz, pos.y + (total_bb.GetHeight() - circle_sz) * 0.5f),
        ImVec2(pos.x + w, pos.y + (total_bb.GetHeight() + circle_sz) * 0.5f)
    );

    bool pressed = ImGui::ButtonBehavior(circle_bb, id, &hovered, &held);

    static std::map<ImGuiID, float> scaleMap;
    if (scaleMap.find(id) == scaleMap.end()) scaleMap[id] = 1.0f;

    if (pressed)
    {
        scaleMap[id] = 1.5f; // pop animation
    ...
    // Animate circle scaling back
    scaleMap[id] = ImLerp(scaleMap[id], 1.0f, g.IO.DeltaTime * 10.0f);

    auto draw = ImGui::GetWindowDrawList();

    // Draw label
    draw->AddText(pos, IM_COL32(255, 255, 255, 255), label);

    // Draw dark backing circle (like custom checkbox)
    ImVec2 circlePos(pos.x + w - circle_sz * 0.5f, pos.y + total_bb.GetHeight() * 0.5f);
    float radius = circle_sz * 0.5f;
    draw->AddCircleFilled(circlePos, radius, IM_COL32(50, 50, 50, 255));

    // Draw white dot in the center that grows on click
    float dotRadius = 3.0f * scaleMap[id]; // tiny dot
    draw->AddCircleFilled(circlePos, dotRadius, IM_COL32(255, 255, 255, 255));
...
void ImGui::SeparatorText2(const char* label, float thickness)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(-0.1f, g.FontSize), label_size.x, g.FontSize);
    ...
    // Draw left line before text (---)
    const float left_line_width = 15.0f;
    if (thickness > 0)
        window->DrawList->AddLine(pos + ImVec2(0, size.y / 2), pos + ImVec2(left_line_width, size.y / 2), GetColorU32(ImGuiCol_Border), thickness);

    // Draw text after left line
    window->DrawList->AddText(nullptr, 15.0f, pos + ImVec2(left_line_width + style.ItemInnerSpacing.x, 0), ImGui::GetColorU32(ImGuiCol_TextDisabled), label);

    // Draw right line after text
    if (thickness > 0)
        window->DrawList->AddLine(pos + ImVec2(left_line_width + style.ItemInnerSpacing.x + label_size.x + style.ItemInnerSpacing.x, size.y / 2), pos + ImVec2(size.x, size.y / 2), GetColorU32(ImGuiCol_Border), thickness);
}
...
struct combo_element {
    float open_anim, arrow_anim, alpha;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(25, 25, 25), 3.0f);
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(25, 25, 25)));
...
bool ImGui::CustomSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 8, label_size.y + 16));
    ...
    auto [it_anim, inserted] = anim.try_emplace(id, slider_element{ 0.4f, 10.0f });
    ...
    it_anim->second.slider_anim = ImLerp(it_anim->second.slider_anim, grab_bb.Min.x - frame_bb.Min.x, ImGui::GetIO().DeltaTime * 15);

    // #909cff
    ImColor newBlue(0x90, 0x9c, 0xff);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(20, 20, 20), 5.0f);
    window->DrawList->AddShadowRect(frame_bb.Min, ImVec2(frame_bb.Min.x + it_anim->second.slider_anim + 9, grab_bb.Max.y + 2),
        ImColor(0x90, 0x9c, 0xff, 200), 20, ImVec2(0, 0), 0, 3.f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + it_anim->second.slider_anim + 9, grab_bb.Max.y + 2), newBlue, 5.0f);
    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slider_anim + 6, grab_bb.Max.y - 1), 5.0f, ImColor(255, 255, 255), 100);
    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slider_anim + 6, grab_bb.Max.y - 1), 3.0f, newBlue, 100);
...
bool ImGui::CustomSliderFloat2(const char* label, float v[2], float v_min, float v_max, const char* format)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float slider_width = ImGui::GetWindowWidth() - 80;
    const float slider_height = 7.0f;
    const ImVec2 pos = window->DC.CursorPos;

    ImGuiID slider_id = window->GetID("slider");
    const ImRect frame_bb(pos, pos + ImVec2(slider_width, slider_height));
    const ImRect total_bb(pos, pos + ImVec2(slider_width + 60, slider_height));

    ImGui::ItemSize(total_bb, 0);
    if (!ImGui::ItemAdd(total_bb, slider_id, &frame_bb))
    {
        ImGui::PopID();
        return false;
    }
    ...
    bool hovered, held;
    bool pressed = ImGui::ButtonBehavior(frame_bb, slider_id, &hovered, &held);

    if (held || pressed)
    {
        float mouse_pos = g.IO.MousePos.x - frame_bb.Min.x;
        float normalized = ImClamp(mouse_pos / frame_bb.GetWidth(), 0.0f, 1.0f);
    ...
    static std::map<ImGuiID, float> slider_anim;
    auto it_anim = slider_anim.find(slider_id);
    if (it_anim == slider_anim.end())
    {
        slider_anim[slider_id] = 0.0f;
        it_anim = slider_anim.find(slider_id);
    }

    float fill_percent = (v[0] - v_min) / (v_max - v_min);
    fill_percent = ImClamp(fill_percent, 0.0f, 1.0f);
    it_anim->second = ImLerp(it_anim->second, fill_percent, ImGui::GetIO().DeltaTime * 8.0f);

    ImColor newBlue(0x90, 0x9c, 0xff);
    ImColor background(20, 20, 20);
    ImColor hover_tint = hovered ? ImColor(200, 200, 200) : ImColor(150, 150, 150);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, background, 7.0f);

    if (it_anim->second > 0.0f)
    {
        float fill_width = frame_bb.GetWidth() * it_anim->second;
        ImRect fill_bb(frame_bb.Min, ImVec2(frame_bb.Min.x + fill_width, frame_bb.Max.y));
        window->DrawList->AddRectFilled(fill_bb.Min, fill_bb.Max, newBlue, 7.0f);
    }

    // Draw value text
    char value_buf[64];
    ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), format, v[0]);
    ImVec2 text_pos = ImVec2(frame_bb.Max.x + 8, frame_bb.Min.y - 1);
    window->DrawList->AddText(nullptr, 13.0f, text_pos, hover_tint, value_buf);
...
{
    return CustomSliderFloat2(label, v, v_min, v_max, format);
}
...
struct InputScalarState {
    ImVec4 TextColor;
    ImVec4 ShadowColor;
    float Shadow_size;
};

bool ImGui::CustomInputScalar(const char* label, const char* hint, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags, float sizeX)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Set the next item width to the full sizeX
    ...
    if (it_anim == anim.end()) {
        anim.insert({ id, InputScalarState() });
        it_anim = anim.find(id);
        it_anim->second.TextColor = ImColor(100, 100, 100);
        it_anim->second.ShadowColor = ImColor(60, 120, 255);
        it_anim->second.Shadow_size = 5.0f;
    }

    // Temporarily modify the style for this input box only
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 5)); // Control vertical padding
    ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0, 0, 0, 0)); // Transparent background

    // Update colors based on interaction
    ImVec4 TextTarget = ImGui::IsItemActive() ? ImColor(255, 255, 255) : ImGui::IsItemHovered() ? ImColor(255, 255, 255) : ImColor(100, 100, 100);
    ImVec4 ShadowTarget = ImGui::IsItemActive() ? ImColor(60, 120, 255) : ImColor(60, 120, 255);
    ...
    ImVec2 inputSize = ImVec2(sizeX, io.Fonts->Fonts[0]->FontSize + style.FramePadding.y * 2);
    ImVec2 maxPos = ImVec2(pos.x + inputSize.x, pos.y + inputSize.y);

    window->DrawList->AddShadowRect(ImVec2(pos.x, maxPos.y - 1), maxPos, ImGui::GetColorU32(it_anim->second.ShadowColor), it_anim->second.Shadow_size, ImVec2(0, 0), 0, 2.0f);
    window->DrawList->AddLine(ImVec2(pos.x, maxPos.y), ImVec2(maxPos.x, maxPos.y), ImColor(60, 120, 255), 1.f);

    // Input box
    flags |= ImGuiInputTextFlags_AutoSelectAll | (ImGuiInputTextFlags)ImGuiInputTextFlags_NoMarkEdited | (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint;

    bool value_changed = false;
    if (p_step == NULL)
    {
        if (InputTextEx(label, hint, buf, IM_ARRAYSIZE(buf), ImVec2(sizeX, 0), flags))
        {
            if (*(int*)p_data == 0)
            {
                buf[0] = 0;
...
bool ImGui::CustomInputInt(const char* label, const char* hint, int* v, int step, int step_fast, ImGuiInputTextFlags flags, float sizeX)
{
    // Format based on hexadecimal flag
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";

    // Call the custom animated input scalar function with an integer-specific setup
    return CustomInputScalar(label, hint, ImGuiDataType_S32, (void*)v,
...
bool ImGui::CustomInputText(const char* label, const char* hint, float sizeX, static char buf[], size_t buf_size, ImGuiInputTextFlags flag)
{
    ImGuiStyle& style = ImGui::GetStyle();
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiIO& io = ImGui::GetIO();
    ...
    window->DrawList->AddShadowRect(ImVec2(pos.x, maxPos.y), maxPos, ImGui::GetColorU32(it_anim->second.ShadowColor), it_anim->second.Shadow_size,ImVec2(0, 0), 0, 1.0f);
    window->DrawList->AddLine(ImVec2(pos.x, maxPos.y), ImVec2(maxPos.x, maxPos.y), ImColor(60, 120, 255), 1.f);

    bool result = ImGui::InputTextEx(label, hint, buf, (int)buf_size, ImVec2(0, 0), flag, 0, 0);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Picker popup at 50% size, no RGB/HSV text labels, no inputs, no side preview bars
    if (!(flags & ImGuiColorEditFlags_NoPicker) && IsPopupOpen("picker", ImGuiPopupFlags_None))
    {
        SetNextWindowSize(ImVec2(g.FontSize * 12.0f, g.FontSize * 12.0f)); // 50% smaller, no extra height
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha;
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoSmallPreview;
                PushItemWidth(g.FontSize * 12.0f - style.FramePadding.x * 2.0f);
    ...
    if (value_changed && memcmp(col, f, sizeof(float) * components) == 0)
    {
        col[0] = f[0];
        col[1] = f[1];
        col[2] = f[2];
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const bool span_all_columns = (flags & ImGuiSelectableFlags_SpanAllColumns | ImGuiWindowFlags_NoDecoration) != 0;
    const float min_x = span_all_columns ? window->ParentWorkRect.Min.x : pos.x;
    const float max_x = span_all_columns ? window->ParentWorkRect.Max.x : window->WorkRect.Max.x;
    if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth | ImGuiWindowFlags_NoDecoration))
    ...
    if ((flags & ImGuiSelectableFlags_NoPadWithHalfSpacing | ImGuiWindowFlags_NoDecoration) == 0)
    {
    ...
    const bool item_add = ItemAdd(bb, id, NULL, disabled_item ? ImGuiItemFlags_Disabled : ImGuiItemFlags_None | ImGuiWindowFlags_NoDecoration);
    if (span_all_columns)
    {
        window->ClipRect.Min.x = backup_clip_rect_min_x;
        window->ClipRect.Max.x = backup_clip_rect_max_x;
    ...
    if (flags & ImGuiSelectableFlags_AllowDoubleClick) { button_flags |= ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnDoubleClick | ImGuiWindowFlags_NoDecoration; }
    ...
    if ((flags & ImGuiSelectableFlags_SelectOnNav) && g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId)
    ...
    if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
    {
        ...
        {
            SetNavID(id, window->DC.NavLayerCurrent, 1, WindowRectAbsToRel(window, bb)); // (bb == NavRect)
            g.NavDisableHighlight = true;
        }
    }
    if (pressed)
    ...
    if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiSelectableFlags_None) && !(g.LastItemData.InFlags))
        CloseCurrentPopup();
...
namespace Fonts {
    inline ImFont* ArrowFont = nullptr;
...
bool ImGui::Tab(bool selected, const char* icon, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.backgroundOpacity = ImLerp(it_anim->second.backgroundOpacity, selected ? 0.8f : (hovered ? 0.0f : 0.0f), g.IO.DeltaTime * 6.f);
    it_anim->second.Icons = ImLerp(it_anim->second.Icons, selected ? ImColor(143, 151, 255) : hovered ? ImColor(143, 151, 255, 150) : ImColor(34, 32, 34), g.IO.DeltaTime * 6.f);
    it_anim->second.Text = ImLerp(it_anim->second.Text, selected ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255, 150) : ImColor(60, 58, 60), g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Max.x - 2, bb.Min.y + 15), 2, ImColor(143 / 255.0f, 151 / 255.0f, 255 / 255.0f, it_anim->second.backgroundOpacity), 100);

    // Push ArrowFont only for the icon
    ImGui::PushFont(Fonts::ArrowFont);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(8, (size_arg.y - CalcTextSize(icon).y + 2) / 2), GetColorU32(it_anim->second.Icons), icon);
    ImGui::PopFont();

    // Draw label with default font
    GetWindowDrawList()->AddText(bb.Min + ImVec2(32, (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.Text), label);

    return pressed;
}


void ImGui::TabDivider(const char* text)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    const ImGuiStyle& style = g.Style;
    const ImVec2 pos = window->DC.CursorPos;
    const float text_height = ImGui::CalcTextSize(text).y;
    const float padding = 10.0f;

    float width = ImGui::GetContentRegionAvail().x;
    ImVec2 line_start = pos + ImVec2(0, text_height / 2);
    ImVec2 line_end = pos + ImVec2(width, text_height / 2);
    window->DrawList->AddLine(line_start, line_end, IM_COL32(60, 58, 60, 100), 1.0f);

    ImVec2 text_pos = pos + ImVec2(padding, 0);
    window->DrawList->AddText(text_pos, IM_COL32(60, 58, 60, 255), text);
    ImGui::ItemSize(ImVec2(width, text_height + padding));
}
...
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.Text = ImLerp(it_anim->second.Text, selected ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255, 160) : ImColor(255, 255, 255, 100), g.IO.DeltaTime * 6.f);
    it_anim->second.Rect = ImLerp(it_anim->second.Rect, selected ? ImColor(20, 18, 20) : hovered ? ImColor(20, 18, 20, 0) : ImColor(18, 16, 18, 0), g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.Rect), 4.f);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(size_arg.x - 52 - CalcTextSize(label).x / 2, (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.Text), label);
